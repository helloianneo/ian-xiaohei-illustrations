...
>
> 16:9 横版 | 熔岩批注怪 IP | 纯白手绘 | 少量红橙蓝中文批注 | Codex Skill
...
默认视觉 IP 是“熔岩批注怪”：一个橙色毛线团身体、白色阶梯脸、深棕细手脚、常拿批注笔的小角色。它不是贴纸，也不是站在角落里的装饰物，而是正在认真参与系统运转的荒诞工作者。
...
- 每张图的主题、核心意思、结构类型、熔岩批注怪动作和中文标注建议
...
这个 skill 默认使用 Ian 的“熔岩批注怪正文配图”风格：
...
```text
Use $ian-xiaohei-illustrations 为这篇中文文章设计并生成 5 张熔岩批注怪正文配图。
```
...
每张图写清楚：放在哪段后、主题、核心意思、结构类型、熔岩批注怪在做什么、建议中文标注词。
...
```text
Use $ian-xiaohei-illustrations 把下面这篇文章生成 4 张熔岩批注怪正文配图。
要求：16:9 横版、纯白背景、黑色/深棕手绘线稿、橙色毛线团角色、少量红橙蓝中文手写批注。
...
6. 让熔岩批注怪承担核心动作
7. 每张图单独调用图像模型生成
8. 按 QA checklist 检查：白底、留白、角色一致性、熔岩批注怪动作、中文标注、非 PPT 感、非旧案例复刻
9. 保存最终 PNG，并报告用途和路径
...
└── ian-xiaohei-illustrations/
    ...
    ├── assets/
    │   ├── ip/
    │   │   └── lava-annotation-mascot-character-sheet.png
    │   └── examples/
    └── references/
        ├── style-dna.md
        ├── lava-annotation-ip.md
...
- 熔岩批注怪必须承担核心动作；如果去掉角色画面仍然完全成立，说明角色太装饰了。
- 角色身份只参考 skill 内的定稿设定图。旧示例图只用于校准线条密度、留白和颜色克制，不要参考旧角色或复刻构图。
...
这套熔岩批注怪配图 Skill，只是我用 AI 搭建个人生产系统里的一个小工具。
