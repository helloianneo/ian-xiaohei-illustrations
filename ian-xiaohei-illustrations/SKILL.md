...
description: 生成 Ian 风格的中文正文配图。用于用户要求为中文文章、帖子、博客、Notion 文档、工作流文档、方法论、流程、结构、状态、隐喻或观点生成“怪诞”“熔岩批注怪”“小黑”“手绘”“正文配图”“文章插图”“配图建议”“shot list”“去标题/改图”等任务；默认使用橙色毛线团造型的熔岩批注怪 IP、纯白手绘、少量红橙蓝批注、简洁清爽但天马行空的视觉风格。
---
...
默认视觉 IP 是“熔岩批注怪”：橙色蜡笔毛线团身体、白色阶梯面部、深棕细手脚，常拿一支橙色批注笔，认真做一件荒诞但成立的事。角色必须参与画面的核心动作，不能只是站在旁边当装饰。
...
- `references/lava-annotation-ip.md`：熔岩批注怪的固定形象、性格、动作库和禁忌。
...
- `assets/ip/lava-annotation-mascot-character-sheet.png`：角色定稿设定图。每次生成含角色的插图前，必须先加载为视觉参考；不得凭文字重新设计角色。
...
生成前先用图像查看工具加载 `assets/ip/lava-annotation-mascot-character-sheet.png`，再把它作为角色身份参考交给图像生成工具。保留白色阶梯面部、橙色自然毛线团、深棕五官和细手脚；允许动作变形，但不要重新设计角色。

不要复刻过往案例。案例只提供风格密度和角色参与方式，不能直接复用“传送带断点 / 角色拉线 / 素材鱼 / 盖章工具箱 / 常见坑路径”等已有构图，除非用户明确要求复刻某张图。每次都要从当前文章重新发明一个奇怪但成立的隐喻。`assets/examples/` 中的旧角色不是身份参考，不得把旧黑色小人带入新图。
