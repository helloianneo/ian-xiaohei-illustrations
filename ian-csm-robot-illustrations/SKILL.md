---
name: ian-csm-robot-illustrations
description: 生成 Ian 风格的中文正文配图。用于用户要求为中文文章、帖子、博客、Notion 文档、工作流文档、方法论、流程、结构、状态、隐喻或观点生成“怪诞”“CSM 机器人”“手绘”“正文配图”“文章插图”“配图建议”“shot list”“去标题/改图”等任务；默认使用 CSM 机器人 IP、纯白手绘、少量红橙蓝批注、简洁清爽但天马行空的视觉风格。
---

# Ian CSM 机器人 怪诞正文配图
> **原始 IP 声明**：本 Skill 基于 Ian 创作的"小黑"（Xiaohei）视觉 IP 角色改造而来。原始角色"小黑"为 Ian 中文正文配图的固定视觉 IP，本项目将其重命名为"CSM 机器人"并进行适配。原作者信息详见 NOTICE.md。

## 核心定位

为中文文章设计和生成 16:9 横版正文配图。目标不是做商业插画、PPT 信息图或可爱卡通，而是把文章里的关键判断、流程、结构、状态或隐喻，变成一张清爽、怪诞、有创意、可读但不说明书的手绘解释图。

默认视觉 IP 是“CSM 机器人”：黑色实心、白点眼、细腿、空表情，认真做一件荒诞但成立的事。 CSM 机器人必须参与画面的核心动作，不能只是站在旁边当装饰。
...
- `references/csm-robot-ip.md`： CSM 机器人 IP 的形象、性格、动作库和禁忌。
...
不要复刻过往案例。案例只提供风格密度和 CSM 机器人参与方式，不能直接复用“传送带断点 / CSM 机器人拉线 / 素材鱼 / 盖章工具箱 / 常见坑路径”等已有构图，除非用户明确要求复刻某张图。每次都要从当前文章重新发明一个奇怪但成立的隐喻。
